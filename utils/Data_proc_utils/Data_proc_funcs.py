...
def balance_child_data(y_df, a_df, print_stats=False, random_state=42):
    ...
    ### 2. Undersample based on the minority class in the children's age group in YLFW
    # - Keep racial distribution as in minority class
    # - Delete based on IDs 
    # (obs be aware that each id have images in different age groups). Therefore we sample based on number of images per age group. 
    ...
    for agegroup in age_labels:
        if agegroup != min_agegroup:
            ...
            for e in etnicities:
                ...
                ylfw_witha_balanced = pd.concat([ylfw_witha_balanced, sample_eids], ignore_index=True).drop(columns=["face_id", "id_class"])
...
# Balance adults data 
def balance_adults_data(y_balanced_df, a_df, print_stats=False, random_state=42):
    """
    Input: raw df rfw and balanced child df
    Returns: csvs with equally balanced children and adults
    Original adults_balanced has random state 42
    """
    
    # Remove childrens age group from adult data
    a_df = a_df[a_df.Age > 18]

    # Undersample adults data based on the ethnicity distributions in balanced child.  
    etnicities = list(y_balanced_df.ethnicity.unique())

    # Save variables of frequency within each ethnicity from y_balanced 
    etnicity_dist = y_balanced_df.groupby('ethnicity').img_name.count().sort_values(ascending=False)
    ...
    for e in etnicities:
        ...
        a_balanced = pd.concat([a_balanced, sample_eids], ignore_index=True).drop(columns=["face_id", "id_class"])
        
    if print:
        print("Balanced data between adults and children?:",
              "\nadults: ", a_balanced.groupby('ethnicity').img_name.count().sort_values(ascending=False), 
              "\n\nchildren: ", y_balanced_df.groupby('ethnicity').img_name.count().sort_values(ascending=False)
        )
